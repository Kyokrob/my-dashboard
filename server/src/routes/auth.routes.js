...
router.get("/me", async (req, res) => {
  ...
  const user = await User.findById(req.session.userId).select(
    "email name role createdAt onboardingStartAt"
  );
  ...
  res.json({
    user: {
      ...
      onboardingStartAt: user.onboardingStartAt,
    },
  });
});
...
// Update profile (name only for MVP)
router.patch("/profile", async (req, res, next) => {
  try {
    ...
    const user = await User.findByIdAndUpdate(
      req.session.userId,
      { name: name.trim() },
      { new: true, select: "email name role createdAt onboardingStartAt" }
    );
    res.json({
      user: {
        id: user.id,
